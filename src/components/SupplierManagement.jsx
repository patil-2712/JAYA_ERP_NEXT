...
const STEPS = [
  ...
  { id: 6, label: "Documents",      icon: FaFileUpload },
...
const DOCUMENT_TYPES = [
  { key: "aadhaarCard", label: "Aadhaar Card", icon: FaIdCard, accept: "image/*,.pdf" },
  { key: "panCard", label: "PAN Card", icon: FaIdCard, accept: "image/*,.pdf" },
  { key: "cancelledCheque", label: "Cancelled Cheque", icon: FaMoneyBillWave, accept: "image/*,.pdf" },
  { key: "visitingCard", label: "Visiting Card", icon: FaRegAddressCard, accept: "image/*,.pdf" },
  { key: "tdsDeclaration", label: "TDS Declaration", icon: HiOutlineDocumentText, accept: "application/pdf" },
];
...
const DocumentUploadSection = ({ documents, onDocumentChange }) => {
  ...
  useEffect(() => {
    DOCUMENT_TYPES.forEach(doc => {
      if (documents[doc.key] && typeof documents[doc.key] === 'string') {
        setPreviews(prev => ({ ...prev, [doc.key]: documents[doc.key] }));
      }
    });
  }, [documents]);
  ...
  const handleFileChange = async (docKey, file) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      toast.error(`${DOCUMENT_TYPES.find(d => d.key === docKey)?.label} exceeds 5MB limit`);
      return;
    }
    ...
    formData.append('section', 'supplier');
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setPreviews(prev => ({ ...prev, [docKey]: response.data.filePath }));
        toast.success(`${DOCUMENT_TYPES.find(d => d.key === docKey)?.label} uploaded successfully`);
      ...
      toast.error(`Failed to upload ${DOCUMENT_TYPES.find(d => d.key === docKey)?.label}`);
    } finally {
  ...
  const removeDocument = (docKey) => {
    ...
    toast.info(`${DOCUMENT_TYPES.find(d => d.key === docKey)?.label} removed`);
  };
    ...
    <div className="space-y-5">
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex items-start gap-2 mb-4">
        ...
        <p className="text-xs text-blue-700 font-medium">
          Select a document type from the radio buttons below, then upload the file.
          <span className="text-gray-500 ml-1">Uploaded: {uploadedCount}/5 documents</span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
        {DOCUMENT_TYPES.map((doc) => (
          ...
          >
            ...
            {documents[doc.key] && (
              <span className="text-[9px] text-emerald-500 mt-1">✓ Uploaded</span>
            )}
          </label>
        ))}
      </div>
        ...
        <div className="border-2 border-indigo-200 rounded-xl p-6 bg-indigo-50/30">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              {DOCUMENT_TYPES.find(d => d.key === selectedDocType)?.icon({ className: "text-indigo-600 text-xl" })}
              <h3 className="font-bold text-indigo-800">
                Upload {DOCUMENT_TYPES.find(d => d.key === selectedDocType)?.label}
              </h3>
            </div>
            <button
              type="button"
            ...
            <div className="space-y-3">
              <div className="bg-white rounded-lg p-3 border border-gray-200">
                <div className="flex items-center gap-3 justify-between">
                  <div className="flex items-center gap-2">
                    {DOCUMENT_TYPES.find(d => d.key === selectedDocType)?.icon({ className: "text-indigo-500" })}
                    <span className="text-sm text-gray-600">File uploaded successfully</span>
                  </div>
                  <button
                    type="button"
                    onClick={() => viewDocument(documents[selectedDocType])}
                    className="px-3 py-1.5 rounded-lg bg-indigo-50 text-indigo-600 text-sm hover:bg-indigo-100"
                  >
              ...
              <div className="flex gap-3">
                <label className="flex-1 cursor-pointer">
                  <div className="w-full py-2.5 rounded-lg bg-indigo-600 text-white text-sm font-semibold text-center hover:bg-indigo-700 transition-all">
                    Change File
                  </div>
                  <input
                    type="file"
                    hidden
                    accept={DOCUMENT_TYPES.find(d => d.key === selectedDocType)?.accept}
                    onChange={(e) => {
                      if (e.target.files?.[0]) handleFileChange(selectedDocType, e.target.files[0]);
                      e.target.value = '';
                    }}
                  />
                </label>
                <button
                  ...
                  className="px-4 py-2.5 rounded-lg bg-red-500 text-white text-sm font-semibold hover:bg-red-600 transition-all"
                >
                ...
                {uploading[selectedDocType] ? (
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-xs text-gray-400">
                      {DOCUMENT_TYPES.find(d => d.key === selectedDocType)?.accept}
                    </span>
                  </div>
                )}
              </div>
      ...
      {uploadedCount > 0 && (
        <div className="mt-4 p-3 bg-gray-100 rounded-lg">
          ...
          <div className="flex flex-wrap gap-2">
            {DOCUMENT_TYPES.filter(doc => documents[doc.key]).map(doc => (
              <span key={doc.key} className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs">
                <doc.icon className="text-[10px]" />
                {doc.label}
                <button
...
export default function SupplierManagement() {
  ...
  const handleDocumentChange = (docKey, filePath) => {
    setSd(prev => ({
      ...prev,
      documents: {
        ...prev.documents,
  ...
  const handleEdit = (s) => {
    setSd({ ...s, glAccount: s.glAccount || null, documents: s.documents || EMPTY.documents });
    ...
    switch (step) {
      ...
      case 6: return (
        <DocumentUploadSection 
          documents={sd.documents || EMPTY.documents}
      ...
      case 7: return (
        <div className="space-y-4">
          ...
          <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
            <p className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-3 flex items-center gap-1.5"><FaFileUpload className="text-indigo-400" /> Documents</p>
            {DOCUMENT_TYPES.map(doc => (
              <RRow 
                key={doc.key}
                l={doc.label}
                v={sd.documents?.[doc.key] ? "✓ Uploaded" : "—"}
              />
            ))}
          </div>
        </div>
                ...
                {loading ? (
                  ...
                  filtered.map(s => (
                    <tr key={s._id} onClick={() => { setViewSupplier(s); setViewOpen(true); }}
                      ...
                      <td className="px-4 py-3">
                        {s.documents && Object.values(s.documents).some(doc => doc) ? (
                          <span className="text-[10.5px] font-semibold px-2 py-0.5 rounded-full bg-green-50 text-green-600">
                            {Object.values(s.documents).filter(doc => doc).length} files
                          </span>
                        ) : (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-2xl bg-white rounded-2xl shadow-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-4">
          ...
          <div>
            <p className="text-[10.5px] font-bold uppercase tracking-wider text-gray-400 mb-2">Documents</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {DOCUMENT_TYPES.map(doc => (
                <div key={doc.key} className="bg-gray-50 rounded-lg p-2 border border-gray-100">
                  <div className="flex items-center gap-2">
                    <doc.icon className="text-indigo-400 text-sm" />
                    <span className="text-[10px] font-medium text-gray-600">{doc.label}</span>
                  </div>
                  <p className="text-[11px] font-semibold mt-1">
                    {s.documents?.[doc.key] ? (
                      <a href={s.documents[doc.key]} target="_blank" rel="noopener noreferrer" className="text-emerald-600 hover:underline">
                        ✓ View File
                      </a>
                    ) : (
