...
//	"NON-UNIFORM - GENERAL CARGO": [defaultRow("NON-UNIFORM - GENERAL CARGO")],
//  });
//
//  /** =========================
...
//  const uom = (updatedRow.uom || "").toUpperCase();
//  
//  let totalPkgs = num(updatedRow.totalPkgs);
//  
//  // Calculate total packages from pallets
...
//	}
//  } else {
//	// Clear calculated fields if insufficient data
...
//  const uom = (updatedRow.uom || "").toUpperCase();
//  
//  console.log("Uniform Calculation:", { totalPkgs, packWeight, uom }); // Debug log
//  
//  if (totalPkgs > 0 && packWeight > 0) {
//	if (uom === "LTR" || uom === "L") {
//	  // For LTR: wtLtr = totalPkgs * packWeight (liters)
//	  const wtLtr = totalPkgs * packWeight;
//	  updatedRow.wtLtr = wtLtr.toFixed(2);
//	  // actualWt = wtLtr / 1000 (convert liters to metric tons)
//	  const actualWt = wtLtr / 1000;
//	  updatedRow.actualWt = actualWt.toFixed(3);
//	} else if (uom === "KG" || uom === "KGS") {
//	  // For KG: actualWt = totalPkgs * packWeight (direct KG)
//	  // NO division by 1000, NO multiplication by 2
//	  const actualWt = totalPkgs * packWeight;
//	  updatedRow.actualWt = actualWt.toFixed(3);
//	  updatedRow.wtLtr = ""; // Clear wtLtr for KG
//	} else {
//	  updatedRow.wtLtr = "";
//	  updatedRow.actualWt = "";
//	}
...
//		
//		// Always recalculate based on pack type
//		if (packType === "PALLETIZATION") {
...
//	"UNIFORM - BAGS/BOXES": [defaultRow("UNIFORM - BAGS/BOXES")],
//	"LOOSE - CARGO": [defaultRow("LOOSE - CARGO")],
//	"NON-UNIFORM - GENERAL CARGO": [defaultRow("NON-UNIFORM - GENERAL CARGO")], // Add this
//  });
//  
//  setSelectedCustomer(null);
...
//	// Call onChange which will trigger recalculation in parent
...
const recalculatePalletizationWeights = (row) => {
  ...
  console.log("Palletization Debug - UOM:", uom, "Raw UOM:", updatedRow.uom);
  ...
  // Calculate weights based on UOM
  if (totalPkgs > 0 && packWeight > 0) {
	// Check for LTR (handle L, LTR, LITRE, LITRES)
	if (uom === "LTR" || uom === "L" || uom === "LITRE" || uom === "LITRES") {
	  const wtLtr = totalPkgs * packWeight;
	  updatedRow.wtLtr = wtLtr.toFixed(2);
	  const actualWt = wtLtr / 1000*2;
	  updatedRow.actualWt = actualWt.toFixed(3);
	  console.log("LTR Calculation - WT LTR:", wtLtr, "ACTUAL WT:", actualWt);
	} 
	// Check for KG (handle KG, KGS, KILOGRAM, KILOGRAMS)
	else if (uom === "KG" || uom === "KGS" || uom === "KILOGRAM" || uom === "KILOGRAMS") {
	  // For KG: actualWt = totalPkgs * packWeight (NO division)
	  const actualWt = totalPkgs * packWeight;
	  updatedRow.actualWt = actualWt.toFixed(3);
	  updatedRow.wtLtr = "";
	  console.log("KG Calculation - ACTUAL WT:", actualWt);
	} else {
	  updatedRow.wtLtr = "";
	  updatedRow.actualWt = "";
	  console.log("Unknown UOM:", uom);
	}
  } else {
	if (!updatedRow.wtLtr) updatedRow.wtLtr = "";
	if (!updatedRow.actualWt) updatedRow.actualWt = "";
...
const recalculateUniformWeights = (row) => {
  ...
  console.log("Uniform Debug - UOM:", uom, "Raw UOM:", updatedRow.uom);
  console.log("Total PKGs:", totalPkgs, "Pack Weight:", packWeight);
  ...
  if (totalPkgs > 0 && packWeight > 0) {
	...
	// Check for KG (handle KG, KGS, KILOGRAM, KILOGRAMS)
	  ...
	  // For KG: actualWt = totalPkgs * packWeight (NO division by 1000)
	  const actualWt = totalPkgs * packWeight;
	  updatedRow.actualWt = actualWt.toFixed(3);
	  updatedRow.wtLtr = "";
	  console.log("KG Calculation - ACTUAL WT:", actualWt);
...
const updatePackRow = (rowId, key, value, packType = activePack) => {
  console.log("Updating row:", rowId, "Key:", key, "Value:", value, "PackType:", packType);
  ...
  setPackData((prev) => {
	const updatedPack = prev[packType].map((r) => {
	  if (r._id === rowId) {
		...
		console.log("After recalculation - ACTUAL WT:", updatedRow.actualWt, "WT LTR:", updatedRow.wtLtr);
...
function PackTypeTable({ 
			...
			rows.map((r) => (
			  <tr key={r._id} className="hover:bg-yellow-50 even:bg-slate-50">
				{cols.map((c) => {
				  ...
				  // For read-only calculated fields (wtLtr, actualWt, totalPkgs)
  ...
  <div className="flex gap-2 items-center">
	...
	{/*<button
	  onClick={onNavigateToCreateUOM}
	  className="rounded-lg bg-green-600 px-2 py-1.5 text-xs font-bold text-white hover:bg-green-700 transition whitespace-nowrap"
