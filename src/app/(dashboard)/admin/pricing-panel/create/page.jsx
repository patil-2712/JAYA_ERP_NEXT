...
//      const data = await res.json();
//      if (data.success && Array.isArray(data.data)) {
//        // Process rate masters to ensure locationRates is properly structured
//        const processedData = data.data.map(rm => ({
//          ...rm,
//          locationRates: rm.locationRates || [],
//          // For backward compatibility, also provide rateSlabs if needed
...
//  const [dropdownPosition, setDropdownPosition] = useState({ top: 0, left: 0, width: 0 });
//
//  // Filter rate masters based on branch ID, customer ID, and location
...
//    }
//    
//    // Filter by location (optional) - Now checking locationRates array
//    if (locationName) {
//      filtered = filtered.filter(rm => {
//        // Check if any locationRate in the array matches the location name
...
//
//  // Get rate from locationRates for a given weight
//  const getRateFromLocationRates = useCallback((rateMaster, weight, locationName) => {
//    if (!rateMaster || !rateMaster.locationRates || !Array.isArray(rateMaster.locationRates)) {
//      return null;
//    }
//    
//    // Find the location rate for this location
//    const locationRate = rateMaster.locationRates.find(lr => lr.locationName === locationName);
//    if (!locationRate) return null;
//    
//    // Check if weight falls within the range
//    const weightNum = parseFloat(weight);
//    if (weightNum >= locationRate.fromQty && weightNum <= locationRate.toQty) {
//      return locationRate.rate;
//    }
//    return null;
//  }, []);
//
...
//      )}
//      
//      {filteredRateMasters.length === 0 && !readOnly && branchId && customerId && locationName && (
//        <div className="text-xs text-amber-600 mt-1">
//          ⚠️ No rate master found for this Branch, Customer, and Location. Please create a rate master first.
//        </div>
//      )}
//    </div>
//  );
...
//    // Find the location rate for this specific location
