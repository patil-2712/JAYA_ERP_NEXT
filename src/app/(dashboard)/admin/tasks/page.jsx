...
export default function TasksPage() {
  ...
  // ── Handlers (Keep your logic, wrap with Modal UI) ──
  const toggleExpand = async (taskId) => {
    if (expandedTaskId === taskId) { setExpandedTaskId(null); } 
    else {
      const token = localStorage.getItem("token");
      const res = await api.get(`/tasks/${taskId}/subtasks`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const openModal = (task = null) => {
    if (task) {
      ...
      setEditTask(null); setTitle(""); setDescription(""); setAssignees([]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const payload = { title, description, assignees, startDate, endDate, priority, status, progress };
    const token = localStorage.getItem("token");
    if (editTask) await api.put(`/tasks/${editTask._id}`, payload, { headers: { Authorization: `Bearer ${token}` } });
    else await api.post("/tasks", payload, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleSubtaskSubmit = async (e) => {
    ...
    const token = localStorage.getItem("token");
    if (editSubtask) await api.put(`/tasks/${editTask._id}/subtasks/${editSubtask._id}`, payload, { headers: { Authorization: `Bearer ${token}` } });
    else await api.post(`/tasks/${editTask._id}/subtasks`, payload, { headers: { Authorization: `Bearer ${token}` } });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">Track company tasks and sub-activities</p>
          </div>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  <th className="px-6 py-4 text-left text-[10.5px] font-bold uppercase tracking-wider text-gray-400 whitespace-nowrap">Task Title</th>
                  <th className="px-6 py-4 text-left text-[10.5px] font-bold uppercase tracking-wider text-gray-400 whitespace-nowrap">Timeline</th>
                  <th className="px-6 py-4 text-left text-[10.5px] font-bold uppercase tracking-wider text-gray-400 whitespace-nowrap">Priority</th>
                  <th className="px-6 py-4 text-left text-[10.5px] font-bold uppercase tracking-wider text-gray-400 whitespace-nowrap">Status</th>
                  <th className="px-6 py-4 text-left text-[10.5px] font-bold uppercase tracking-wider text-gray-400 whitespace-nowrap w-20 text-center">Progress</th>
                ...
                {tasks.map((t) => (
                  <React.Fragment key={t._id}>
                    <tr onClick={() => toggleExpand(t._id)} className="hover:bg-indigo-50/30 transition-colors cursor-pointer group">
                      ...
                      <td className="px-6 py-4"><StatusBadge state={t.status} /></td>
                      <td className="px-6 py-4">
                        <div className="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden">
                          <div className="bg-indigo-500 h-full transition-all" style={{ width: `${t.progress}%` }} />
                        </div>
                        <p className="text-[9px] font-black text-indigo-400 mt-1 text-center">{t.progress}%</p>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <button onClick={(e) => { e.stopPropagation(); setOpenMenuId(openMenuId === t._id ? null : t._id); }} className="p-2 text-gray-300 hover:text-indigo-600">⋮</button>
                        {openMenuId === t._id && (
                          <div className="absolute right-12 bg-white border border-gray-100 rounded-xl shadow-xl z-20 w-40 text-left overflow-hidden">
                            <button onClick={() => openSubtaskModal(t)} className="flex items-center gap-2 w-full px-4 py-2.5 text-xs text-emerald-600 hover:bg-emerald-50 font-bold"><FaPlus /> Add Subtask</button>
                            ...
                            <button onClick={() => { if(confirm("Delete?")) api.delete(`/tasks/${t._id}`).then(() => fetchData()); }} className="flex items-center gap-2 w-full px-4 py-2.5 text-xs text-red-600 hover:bg-red-50 font-bold"><FaTrash /> Delete</button>
                          </div>
                        )}
                      </td>
                    </tr>
                    ...
                    {expandedTaskId === t._id && t.subtasks?.map(s => (
                      <tr key={s._id} className="bg-gray-50/50 border-l-4 border-indigo-400">
                        ...
                        <td className="px-6 py-3">
                           <div className="w-full bg-gray-200 h-1 rounded-full"><div className="bg-indigo-400 h-full" style={{width:`${s.progress}%`}} /></div>
                        </td>
        ...
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-indigo-50/50">
              <h2 className="text-xl font-black text-gray-900">{editTask ? "Update Task" : "Create New Task"}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-8 overflow-y-auto space-y-5">
              <div><Lbl text="Task Title" /><input type="text" className={fi} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Website Redesign" required /></div>
              <div><Lbl text="Description" /><textarea className={`${fi} h-24 resize-none`} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Task details..." /></div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><Lbl text="Status" /><select className={fi} value={status} onChange={(e) => setStatus(e.target.value)}><option value="todo">To Do</option><option value="in-progress">In Progress</option><option value="done">Done</option></select></div>
              </div>
              <div>
                <Lbl text="Assignees" />
              ...
              <div>
                <Lbl text={`Completion Progress (${progress}%)`} />
                <input type="range" className="w-full accent-indigo-600" value={progress} onChange={(e) => setProgress(e.target.value)} />
              </div>
              <div className="flex justify-end gap-3 pt-4">
                <button type="button" onClick={() => setModalOpen(false)} className="px-6 py-2.5 rounded-xl border border-gray-200 text-gray-500 font-bold text-sm">Cancel</button>
        ...
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden flex flex-col">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-emerald-50/50">
              <h2 className="text-xl font-black text-gray-900">Add Activity to "{editTask?.title}"</h2>
            </div>
            <form onSubmit={handleSubtaskSubmit} className="p-8 space-y-5">
              ...
              <div className="flex justify-end gap-3">
                <button type="button" onClick={() => setSubModalOpen(false)} className="px-6 py-2.5 rounded-xl border border-gray-200 text-gray-500 font-bold text-sm">Close</button>
