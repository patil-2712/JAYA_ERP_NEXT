...
const addressSchema = new mongoose.Schema({
  title: { type: String, trim: true, default: "" },  // ✅ Address Title
  address1: { type: String, trim: true },
  address2: { type: String, trim: true },
  city: { type: String, trim: true },
  state: { type: String, trim: true },
...
const documentSchema = new mongoose.Schema({
  ...
  iecCertificate: { type: String, default: null },
  otherDocuments: [{ name: String, filePath: String, uploadedAt: Date }]
}, { _id: false });
...
const customerSchema = new mongoose.Schema({
  ...
  createdBy: { type: mongoose.Schema.Types.ObjectId, ref: 'CompanyUser' },
  assignedAgents: [{ type: mongoose.Schema.Types.ObjectId, ref: 'CompanyUser' }],
  lastAssignedAgentIndex: { type: Number, default: -1 },
  customerCode: { type: String, trim: true, uppercase: true },
  customerName: { type: String, required: true, trim: true },
  customerGroup: { type: String, required: true, trim: true },
  customerType: { type: String, required: true, enum: ['Individual', 'Business', 'Government', 'E-commerce'], default: 'Individual' },
  emailId: { type: String, match: [/^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/, "Invalid email"] },
  password: { type: String, select: false },
  portalAccess: { type: Boolean, default: true },
  contactEmails: [{ email: String, name: String, designation: String, password: String, isPrimary: Boolean }],
  fromLead: { type: String, trim: true },
  mobileNumber: { type: String, match: [/^[0-9]{10}$/, "Invalid mobile"] },
  fromOpportunity: { type: String, trim: true },
  billingAddresses: [addressSchema],
  shippingAddresses: [addressSchema],
  paymentTerms: { type: String, trim: true },
  ...
  contactPersonName: { type: String, trim: true },
  commissionRate: { type: String, trim: true },
  glAccount: { type: mongoose.Schema.Types.ObjectId, ref: "BankHead" },
  slaPolicyId: { type: mongoose.Schema.Types.ObjectId, ref: "SlaPolicy" },
  documents: { type: documentSchema, default: { gstCertificate: null, panCard: null, iecCertificate: null, otherDocuments: [] } }
...
customerSchema.post("save", function (error, doc, next) {
  if (error.name === "MongoServerError" && error.code === 11000) {
    if (error.keyPattern?.companyId && error.keyPattern?.customerCode) {
      next(new Error(`Customer code "${error.keyValue?.customerCode}" already exists`));
    } else if (error.keyPattern?.companyId && error.keyPattern?.emailId) {
      next(new Error(`Email "${error.keyValue?.emailId}" already registered`));
    } else {
